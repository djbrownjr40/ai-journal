...
export default defineComponent({
  ...
  setup() {
    ...
    const loadEntry = async (): Promise<void> => {
      console.log('Loading entry with ID:', entryId.value);

      if (entryId.value) {
        try {
          const token = localStorage.getItem('token');
          const response = await axios.get(`http://157.7.86.182:3000/api/entries/${entryId.value}`, {
            headers: {
          ...
          entryTitle.value = response.data.title || 'Untitled';
    ...
    const saveEntry = async (): Promise<void> => {
      try {
        ...
        await axios.put(`http://157.7.86.182:3000/api/entries/${entryId.value}`,
    ...
    onMounted(async () => {
      await loadEntry(); // Load entry on mount
